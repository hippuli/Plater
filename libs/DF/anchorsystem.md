...
damageAnchors:CreateScreenAnchor("damage", "DamagePivot", "bottom", ...)
...
healingAnchors:CreateScreenAnchor("healing", "HealingPivot", "right", ...)
...
### `CreateScreenAnchor(anchorKey, frameName, growDirection, setupFunction, sortFunction, frameCreateFunction)`

Creates a new screen anchor and registers it with the anchor system.

Parameters:
- `anchorKey`: unique key used to identify this anchor in the profile table.
- `frameName`: the frame name passed to `CreateFrame`.
- `growDirection`: the direction frames grow from the anchor (\"top\", \"bottom\", \"left\", \"right\").
- `setupFunction`: called for each frame when it is acquired from the pool.
- `sortFunction`: used to order frames inside this anchor.
- `frameCreateFunction`: factory used by the frame pool to create new child frames.
